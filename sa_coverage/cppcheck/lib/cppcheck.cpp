...
unsigned int CppCheck::check(const std::string &path)
{
    ...
    // std::cout << "SOECM: CHECKING FILE " << path << std::endl;
...
unsigned int CppCheck::checkFile(const std::string& filename, const std::string &cfgname, std::istream& fileStream)
{
	...
	//std::cout << "SOECM: TERMINATION" << std::endl;
        return mExitCode;
    }
    ...
    //std::cout << "SOECM: CHECKING UNUSED FUNCTIONS" << std::endl;
    ...
    try {
	    ...
	    std::cout << "SOECM: Iterating over output." << std::endl;
            bool err;
            switch (output.type) {
        ...
        if (mSettings.checkConfiguration) {
            for (const std::string &config : configurations) {  // SOECM
		// std::cout << filename << std::endl;  // SOECM
	    ...
	    // std::cout << "SOECM CFG: " << currCfg << std::endl;  // SOECM
	...
	std::cout << "SOECM: RUNTIME ERROR??" << std::endl;
...
void CppCheck::checkNormalTokens(const Tokenizer &tokenizer)
{
    ...
    std::cout << "~~~ FILE TOKENS: ~~~" << std::endl;
    for (const Token* tok = tokenizer.tokens(); tok; tok = tok->next()) {
        std::string tokStr = tok->str();

        std::cout << tokStr << " ";
        /*if (tokStr.find(semiColon) != std::string::npos) {
            std::cout << std::endl;
        }*/
    }
    std::cout << std::endl;
    ...
    for (Check *check : Check::instances()) {
        ...
        // std::cout << "Running checks for " << check->name() << std::endl;  // SOECM
	numChecksDone++; // SOECM
        check->runChecks(&tokenizer, &mSettings, this);
    }
    std::cout << "~~~ Ran " << numChecksDone << " checks for this above chunk ~~~" << std::endl << std::endl;  // SOECM
